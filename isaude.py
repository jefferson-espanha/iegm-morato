...
# =============================================================================
                    # MÓDULO DE INDICADORES SUPLEMENTARES - QUESITO S19 (SISAB / APS)
                    # =============================================================================

                    # -----------------------------------------------------------------------------
                    # QUESITO S19 (Acompanhamento de Diabéticos na APS - SISAB)
                    # -----------------------------------------------------------------------------
                    with ui.card().classes("w-full p-6 mb-6 border border-gray-300 rounded-lg shadow-sm bg-white"):
                        ui.label("S19 • Acompanhamento de Pessoas com Diabetes na APS (SISAB)").classes("text-xl font-semibold text-blue-600 mb-2")
                        ui.label(
                            "Informe o número de diabéticos com consulta em DM e solicitação de Hemoglobina Glicada nos últimos 6 meses (DHG) "
                            "e o total de diabéticos cadastrados (TD) para os 3 quadrimestres de 2025 (SISAB):"
                        ).classes("text-sm text-gray-700 mb-4")
                        ...
                        lbl_pct_s19 = ui.label("").classes("text-base font-semibold text-blue-800 mb-1")
                        lbl_pontos_s19 = ui.label("").classes("text-base font-bold mb-4")

                        def calc_s19(d1, d2, d3, t1, t2, t3):
                            soma_dhg = d1 + d2 + d3
                            soma_td = t1 + t2 + t3
                        ...
                        def atualizar_calculo_s19():
                            ...
                            if soma_td > 0:
                                lbl_pct_s19.set_text(f"• Percentual Acumulado (P): {pct:.2f}%")
                                lbl_pontos_s19.set_text(f"Pontuação Calculada: {pts:.1f} / 25.0 pontos")
                                
                                if pts >= 15.0:
                                    lbl_pontos_s19.classes(remove="text-red-600 text-yellow-600", add="text-green-600")
                                elif pts >= 5.0:
                                    lbl_pontos_s19.classes(remove="text-red-600 text-green-600", add="text-yellow-600")
                                else:
                                    lbl_pontos_s19.classes(remove="text-green-600 text-yellow-600", add="text-red-600")
                            else:
                                lbl_pct_s19.set_text("• Percentual Acumulado (P): Informe o Total de Diabéticos (TD)")
                        ...
                        ui.textarea(
                            label="Link / Comprovação dos dados do SISAB:",
                            value=state_s19["link"]
                        ).classes("w-full mb-3").props("outlined dense rows=2").bind_value(state_s19, "link")
                        ...
                        def salvar_s19():
                            ...
                            dados_finais = {
                                "dhg1q": d1, "dhg2q": d2, "dhg3q": d3,
                                "td1q": t1, "td2q": t2, "td3q": t3,
                                "proporcao": round(prop, 4),
                                "percentual": round(prop * 100.0, 2)
                            }
                            ...
                            save_resposta(
                                ...
                                pontos=pts,
                                link=state_s19["link"],
                                comentarios=ds19.get("comentarios", []),
                                status=ds19.get("status", "Pendente")
                            )
                            ui.notify("Quesito S19 salvo com sucesso!", type="positive")
                            if render_conteudo.refresh:
                                render_conteudo.refresh()

                        ui.button("💾 SALVAR QUESITO S19", on_click=salvar_s19).classes("bg-blue-600 text-white font-bold my-2")
                        ui.separator().classes("my-2")
