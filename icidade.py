...
# =============================================================================
# 2. SCHEMAS E COMPONENTES DE INTERFACE (STREAMLIT)
# =============================================================================


def renderizar_quesito_c11(ano_sel, res_data, container_formulario_icidade):
    """Renderiza a interface do quesito C1.1 dentro do fluxo do Streamlit."""
    opcoes_c11 = {
        "Selecione...": 0.0,
        "Etapa A (10 pts)": 10.0,
        "Etapa B (20 pts)": 20.0,
...
def gerar_relatorio_pdf(dados, ano, total, faixa):
    ...
    dados_sumario = [
        [
            Paragraph("1. Resumo Executivo (Análise Comparativa)", style_item_esquerda),
            Paragraph("Pág. 3", style_pag_direita),
        ],
        [
            Paragraph("2. Análise de Desempenho por Quesito", style_item_esquerda),
            Paragraph("Pág. 3", style_pag_direita),
        ],
        [
        ...
        [
            Paragraph("5. Alinhamento com a Agenda 2030 (ODS)", style_item_esquerda),
            Paragraph("Pág. 4", style_pag_direita),
        ],
        [
        ...
        [
            Paragraph("7. Quesitos Sem Pontuação Direta", style_item_esquerda),
            Paragraph("Pág. 5", style_pag_direita),
        ],
    ]
    ...
    dados_comparativos = [
        ...
        [
            ...
            Paragraph(f"{seta_tendencia} {variacao_pontos:+.1f} pts", style_td_var),
            Paragraph(f"{seta_tendencia} {texto_percentual}", style_td_var),
        ],
    ]
    ...
    for qid, info in dados.items():
        ...
        if qid == "1.0":
            ...
            metas = "11.5, 16.6"
            status = "Não Atendido" if "não atuam de forma sistêmica" in resp_l else "Atendido"
        elif qid == "2.0":
            metas = "11.5, 16.6"
            status = "Atendido" if "sim" in resp_l else "Não Atendido"
        elif qid == "3.0":
            ...
            status = "Atendido" if ("sim, integralmente" in resp_l or "sim, parcialmente" in resp_l) else "Não Atendido"
        elif qid == "5.1.1.1":
            metas = "11b, 11.5, 16.6"
            status = "Atendido" if "sim" in resp_l else "Não Atendido"
        elif qid == "5.1.2":
            ...
            metas = "11b, 11.5, 16.6"
            status = "Atendido" if ("sim" in resp_l or "parcialmente" in resp_l) else "Não Atendido"
        elif qid == "7.0":
            metas = "11b, 11.5, 16.6"
            status = "Atendido" if "sim" in resp_l else "Não Atendido"
        elif qid == "7.3":
        ...
        if metas:
            analise_ods.append([qid, metas, status])

    if analise_ods:
        data_ods = [["Quesito", "Metas ODS Associadas", "Status de Conformidade"]] + analise_ods
        tabela_ods = Table(data_ods, colWidths=[80, 210, 190])
        tabela_ods.setStyle(
            TableStyle([
