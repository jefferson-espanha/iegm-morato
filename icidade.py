...
def converter_pontos_em_faixa_iegm(pontos):
  """Converte a pontuação no conceito/faixa do IEGM."""
  pts = float(pontos)
  if pts < 500.0:
    return "C"
...
def gerar_relatorio_pdf_bytes(
    dados, ano, total, faixa, fn_obter_dados_historicos=None
):
  """Gera o relatório em PDF e retorna os bytes bufferizados para o NiceGUI."""
  buffer = BytesIO()
  ...
  dados_sumario = [
      [
          Paragraph(
              "1. Resumo Executivo (Análise Comparativa)", style_item_esq
          ),
          Paragraph("Pág. 3", style_pag_dir),
      ],
      [
          Paragraph("2. Análise de Desempenho por Quesito", style_item_esq),
          Paragraph("Pág. 3", style_pag_dir),
      ],
      [
          Paragraph("3. Análise de Impacto e Penalidades", style_item_esq),
          Paragraph("Pág. 4", style_pag_dir),
      ],
      [
          Paragraph("4. Diagnóstico de Reincidências", style_item_esq),
          Paragraph("Pág. 4", style_pag_dir),
      ],
      [
          Paragraph("5. Alinhamento com a Agenda 2030 (ODS)", style_item_esq),
          Paragraph("Pág. 4", style_pag_dir),
      ],
      [
          Paragraph("6. Série Histórica do I-cidade", style_item_esq),
          Paragraph("Pág. 5", style_pag_dir),
      ],
      [
          Paragraph(
              "7. Quesitos Sem Pontuação Direta", style_item_esq
          ),
          Paragraph("Pág. 5", style_pag_dir),
      ],
  ]
