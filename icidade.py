...
def render_quesito(
    ...
    tipo="radio",  # 'radio', 'checkbox', 'multiselect', 'text_input'
    informativo=False,
    is_text_area=False,
    placeholder_text="Cole os links das evidências aqui...",
    placeholder_link="Cole os links de comprovação ou observações...",
    ...
    **kwargs,
    ...
    container_key = f"container_bloco_{qid}_{ano}".replace(".", "_")

    with st.container(key=container_key, border=True):
        with st.expander(f"📌 Quesito {qid} - {titulo}", expanded=True):
            st.subheader(f"{qid} • {titulo}")
            st.write(f"**{pergunta}**")
            st.caption(
            ...
            d_data = res_data.get(qid) or {
                "valor": (
                    ...
                    if tipo == "checkbox"
                        ...
                        "Selecione..."
                        if isinstance(opcoes, dict) and opcoes
                        else ""
                    )
                ),
                "pontos": 0.0,
            ...
            col_input, col_link = st.columns([1, 1])

            # -----------------------------------------------------------------
            # 1. TRATAMENTO DO TIPO CHECKLIST / CHECKBOX (MÚLTIPLA ESCOLHA)
            # -----------------------------------------------------------------
            if tipo == "checkbox":
                # Converte a string salva de volta para lista
                if isinstance(valor_salvo, str):
                    try:
                        valor_selecionado = ast.literal_eval(valor_salvo)
                        if not isinstance(valor_selecionado, list):
                            valor_selecionado = []
                    except Exception:
                        valor_selecionado = []
                elif isinstance(valor_salvo, list):
                    valor_selecionado = valor_salvo
                else:
                    valor_selecionado = []
                ...
                with col_input:
                    # Gera um checkbox independente para cada opção
                    for opt in lista_opcoes:
                        st.checkbox(
                            opt,
                            value=(opt in valor_selecionado),
                            key=f"chk_{qid}_{opt}_{ano}".replace(" ", "_"),
                        )

            # -----------------------------------------------------------------
            # 2. TRATAMENTO DO TIPO RADIO (ESCOLHA ÚNICA)
            # -----------------------------------------------------------------
            elif tipo == "radio":
                lista_opcoes = (
                    list(opcoes.keys()) if isinstance(opcoes, dict) else opcoes
                )
                idx_sel = 0
                if valor_salvo in lista_opcoes:
                    idx_sel = lista_opcoes.index(valor_salvo)
                ...
                with col_input:
                    st.radio(
                        ...
                        index=idx_sel,
                        key=f"rad_{qid}_{ano}".replace(".", "_"),
                    )

            # -----------------------------------------------------------------
            # ÁREA DE EVIDÊNCIA / LINKS
            # -----------------------------------------------------------------
            with col_link:
                ...
                link_input = st.text_area(
                    ...
                    key=f"l_{qid}_txt_{ano}".replace(".", "_"),
                    height=185 if tipo == "checkbox" else 120,
                    placeholder=ph_link,
                )

            # -----------------------------------------------------------------
            # BOTÃO SALVAR E PROCESSAMENTO DOS DADOS
            # -----------------------------------------------------------------
            if st.button(
                f"💾 Salvar Quesito {qid}",
                key=f"btn_salvar_{qid}_{ano}".replace(".", "_"),
                type="primary",
            ):
                # Coleta valor final de acordo com o tipo
                if tipo == "checkbox":
                    lista_opcoes = (
                        list(opcoes.keys())
                        if isinstance(opcoes, dict)
                    ...
                    selecionados = [
                        ...
                        if st.session_state.get(
                            f"chk_{qid}_{opt}_{ano}".replace(" ", "_"), False
                        )
                    ]
                    valor_final = str(selecionados)
                elif tipo == "radio":
                    valor_final = st.session_state.get(
                        f"rad_{qid}_{ano}".replace(".", "_"), ""
                    )
                else:
                    valor_final = ""
                ...
                # Atualiza memória e res_data
                if qid not in res_data:
                    res_data[qid] = {}
                res_data[qid]["valor"] = valor_final
                res_data[qid]["pontos"] = pts_calc
                res_data[qid]["link"] = link_input

                # Chama a função global de persistência se existir
                if "save_resp" in globals():
                    coments_atuais = d_data.get("comentarios", [])
                    globals()["save_resp"](
                        qid=qid,
                        valor=valor_final,
                        pontos=pts_calc,
            ...
            # Exibição do impacto na pontuação
            st.markdown(
                f"<span style='color:#28a745; font-weight:bold;'>"
                f"📊 Impacto de Pontuação no Quesito {qid}: {d_data.get('pontos', 0.0)} pontos</span>",
